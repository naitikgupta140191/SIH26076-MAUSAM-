...
export const EventPlannerModal: React.FC<EventPlannerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-md animate-fadeIn">
      <div className="glass-card w-full max-w-2xl rounded-3xl p-6 border border-amber-200 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-amber-100 pb-4 mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-800">Event Weather Intelligence & Comfort Index</h3>
              <p className="text-xs text-slate-700">Tailored planner matrix for weddings, concerts & gatherings</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          <div>
            <label className="text-xs text-slate-700 font-medium">Event Type</label>
            <select
              value={eventType}
              onChange={(e) => setEventType(e.target.value)}
              className="w-full mt-1 bg-white border border-amber-200 rounded-xl px-3 py-2 text-xs text-slate-700 focus:outline-none focus:border-amber-400"
            >
          ...
          <div>
            <label className="text-xs text-slate-700 font-medium">Estimated Guests</label>
            <input
              type="number"
              value={guestCount}
          ...
          <div>
            <label className="text-xs text-slate-700 font-medium">Event Date</label>
            <input
              type="date"
              value={eventDate}
        ...
        <div className="glass-card rounded-2xl p-6 border border-amber-200 bg-amber-50/60 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-700 font-mono">{eventType} in {currentCity}</span>
            </div>
            <Sparkles className="w-5 h-5 text-amber-700" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="p-3.5 rounded-xl bg-white border border-amber-100">
              <div className="text-xs text-slate-700">Peak Rain Probability</div>
              <div className="text-xl font-bold text-amber-700 flex items-center gap-1 my-1">
                <CloudRain className="w-4 h-4" /> {rainProb}%
              </div>
              <div className="text-[10px] text-slate-700">Low cancellation hazard</div>
            </div>

            <div className="p-3.5 rounded-xl bg-white border border-amber-100">
              <div className="text-xs text-slate-700">Outdoor Comfort Index</div>
              <div className="text-xl font-bold text-emerald-600 flex items-center gap-1 my-1">
                <Sun className="w-4 h-4" /> {humidex}°C
              </div>
              <div className="text-[10px] text-slate-700">Humidex perceived temp</div>
            </div>

            <div className="p-3.5 rounded-xl bg-white border border-amber-100">
              <div className="text-xs text-slate-700">Wind Stability</div>
              <div className="text-xl font-bold text-amber-700 flex items-center gap-1 my-1">
                <Wind className="w-4 h-4" /> 14 km/h
              </div>
              <div className="text-[10px] text-slate-700">Safe for high canopy tents</div>
            </div>
          </div>

          <div className="space-y-2 pt-2">
            <h4 className="text-xs font-bold uppercase tracking-wider text-amber-700">Planner Recommendations</h4>
