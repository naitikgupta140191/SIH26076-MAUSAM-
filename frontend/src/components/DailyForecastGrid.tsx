  ...
  return (
    <div className="glass-card rounded-2xl p-6 border border-slate-800 my-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-slate-400 font-mono">High Precision Daily Forecast</span>
      </div>
          ...
          return (
            <div
              ...
              className={`p-4 rounded-xl glass-card border transition-all duration-300 flex flex-col items-center justify-between text-center ${
                idx === 0 ? 'border-cyan-500/50 bg-slate-900/90 shadow-md' : 'border-slate-800/80 hover:border-slate-700'
              }`}
            >
              <span className={`text-xs font-bold uppercase tracking-wider ${idx === 0 ? 'text-cyan-400' : 'text-slate-300'}`}>
                {dayName}
              </span>

              <div className="my-3 p-2.5 rounded-full bg-slate-800/60 text-cyan-300">
                {hasRain ? (
              ...
              <div className="space-y-1 w-full">
                <div className="flex items-center justify-center gap-2 text-sm font-extrabold text-slate-100">
                  <span className="text-slate-100">{Math.round(day.temp_max)}°</span>
                  <span className="text-slate-500 text-xs">/</span>
                  <span className="text-slate-400 font-normal text-xs">{Math.round(day.temp_min)}°</span>
                </div>
